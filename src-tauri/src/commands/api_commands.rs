...
pub async fn create_billing_portal_session(
    ...
    if !account.is_devin() {
        return Ok("https://windsurf.com/account".to_string());
    }
    ...
    let auth = devin_service
        .post_auth(auth1_token)
        .await
        .map_err(|e: AppError| e.to_string())?;
    devin_service
        .create_billing_portal_session(auth1_token, &auth.org_id, &auth.org_name)
        .await
        .map_err(|e: AppError| e.to_string())
...
pub async fn fetch_stripe_portal_billing(
    ...
    let auth1_token = account.refresh_token
        ...
        .ok_or_else(|| "Missing auth1 token for Stripe Portal billing".to_string())?;
    ...
    windsurf_service
        ...
        .fetch_stripe_portal_billing_data(&portal_url)
...
pub async fn cancel_subscription(
    ...
    let has_auth1_token = account
        .refresh_token
        .as_deref()
        .map(|t| t.starts_with("auth1_"))
        .unwrap_or(false);

    let windsurf_service = WindsurfService::new();

    let result: serde_json::Value = if is_devin_account && has_auth1_token {
        let auth1_token = account
            .refresh_token
            .as_deref()
            .ok_or_else(|| "auth1 refresh_token missing".to_string())?;
        let devin_service = DevinService::new();
        let auth = devin_service
            .post_auth(auth1_token)
            .await
            .map_err(|e: AppError| format!("Failed to get Devin organization info: {}", e))?;
        let portal_url = devin_service
            .create_billing_portal_session(auth1_token, &auth.org_id, &auth.org_name)
            .await
            .map_err(|e: AppError| format!("Failed to get Stripe Portal URL: {}", e))?;
        windsurf_service
            .cancel_stripe_portal_subscription(&portal_url, false)
            .await
            .map_err(|e: AppError| e.to_string())?
    } else {
        ensure_valid_token(&store, &mut account, uuid).await?;
        ...
        let auth1_token = account
            ...
            .filter(|token| token.starts_with("auth1_"));
        windsurf_service
            .cancel_plan(&token, auth1_token, &reason)
            .await
            .map_err(|e: AppError| e.to_string())?
    };

    let log_account = store.get_account(uuid).await.ok();
    if let Some(acc) = &log_account {
        let success = result.get("success").and_then(|v| v.as_bool()).unwrap_or(false);
        let channel = if is_devin_account && has_auth1_token { "Stripe Portal" } else { "CancelPlan" };
        let log = OperationLog::new(
            ...
            format!("Cancel subscription ({}, reason: {}): {}", channel, reason, acc.email),
        )
...
#[tauri::command]
pub async fn cancel_windsurf_subscription(
    id: String,
    reason: String,
    store: State<'_, Arc<DataStore>>,
) -> Result<serde_json::Value, String> {
    ...
    let mut account = store.get_account(uuid)
        ...
        .map_err(|e| e.to_string())?;
    if account.is_devin() {
        return Err("Current account is Devin source; Windsurf-only cancellation skipped".to_string());
    }
    ...
    let windsurf_service = WindsurfService::new();
    let result: serde_json::Value = windsurf_service
        .cancel_plan(&token, auth1_token, &reason)
        .await
        .map_err(|e: AppError| e.to_string())?;
    ...
    if let Some(acc) = &log_account {
        ...
        let log = OperationLog::new(
            ...
            format!("Cancel Windsurf subscription (reason: {}): {}", reason, acc.email),
        )
        .with_account(uuid, acc.email.clone());
